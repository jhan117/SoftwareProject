...
void replaceNumbers(int board[SIZE][SIZE]) {
    int mapping[10];
    for (int i = 1; i <= 9; i++) mapping[i] = i;

    for (int i = 1; i <= 9; i++) {
        int j = 1 + rand() % 9;
        int tmp = mapping[i]; mapping[i] = mapping[j]; mapping[j] = tmp;
    }
    for (int i = 0; i < SIZE; i++)
        for (int j = 0; j < SIZE; j++)
...
void generateRandomSudoku(int board[SIZE][SIZE], int level) {
    ...
    switch (level) {
    case 1:
        min_ops = 10; max_ops = 15; blanks = 30; break;
    case 2:
        min_ops = 15; max_ops = 25; blanks = 40; break;
    case 3:
        min_ops = 20; max_ops = 35; blanks = 50; break;
    default:
        min_ops = 15; max_ops = 25; blanks = 40; break;
    }
    ...
    for (int i = 0; i < ops; i++) {
        ...
        switch (operation) {
        case 0: swapRandomRowsInBlock(board); break;
        case 1: swapRandomColsInBlock(board); break;
        case 2: swapRandomRowBlocks(board); break;
        case 3: swapRandomColBlocks(board); break;
        case 4: rotateBoard(board); break;
        case 5: mirrorBoard(board); break;
        case 6: replaceNumbers(board); break;
        }
    }
