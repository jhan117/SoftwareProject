        ...
        for (int j = 0; j < SIZE; j++) {
          ...
          for (int n = 0; n < SIZE; n++)
            board->cells[i][j].candidates[n] =
                !(rowUsed[i][n] || colUsed[j][n] ||
                  boxUsed[(i / 3) * 3 + (j / 3)][n]);
        }
    }
}

static int isValid(SudokuBoard *board, int row, int col, int num) {
  for (int i = 0; i < SIZE; i++) {
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      if (board->cells[startRow + i][startCol + j].value == num)
        return 0;
  return 1;
}

static Cell *MRV(SudokuBoard *board, int *out_row, int *out_col) {
...
static int removeCandidates(SudokuBoard *board, int row, int col, int num,
                            CandidateChange stack[]) {
  int n = num - 1;
  int sr = row - row % 3;
  int sc = col - col % 3;
  int top = 0;
  ...
  for (int i = 0; i < SIZE; i++) {
    if (board->cells[row][i].value == 0) {
      stack[top++] =
          (CandidateChange){row, i, num, board->cells[row][i].candidates[n]};
      board->cells[row][i].candidates[n] = 0;
    }
    if (board->cells[i][col].value == 0) {
      stack[top++] =
          (CandidateChange){i, col, num, board->cells[i][col].candidates[n]};
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++) {
      ...
      if (board->cells[r][c].value == 0) {
        stack[top++] =
            (CandidateChange){r, c, num, board->cells[r][c].candidates[n]};
...
static void restoreCandidates(SudokuBoard *board, CandidateChange stack[],
                              int top) {
  while (top > 0) {
...
int solveSudokuHDFS(SudokuBoard *board, SudokuSolutions *sols,
  ...
  if (board->empty_count == 0) {
    if (sols->count < MAX_SOLUTIONS) {
      ...
      sols->hdfs_calls[sols->count] = *curr_calls;
      sols->count++;
    }
    return 1;
  }
  ...
  for (int n = 0; n < SIZE; n++) {
    ...
    solveSudokuHDFS(board, sols, curr_calls);
