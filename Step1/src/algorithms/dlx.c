...
static void addRow(DLX *dlx, int row_id, int *col_indices, int col_num) {
  Node *rowNodes[4];
...
void boardToDLX(DLX *dlx, int board[SIZE][SIZE]) {
  for (int r = 0; r < SIZE; r++) {
    for (int c = 0; c < SIZE; c++) {
      if (board[r][c] != 0) {
        int num = board[r][c];
        int cols[4] = {r * SIZE + c, SIZE * SIZE + r * SIZE + (num - 1),
                       (SIZE * SIZE * 2) + c * SIZE + (num - 1),
                       (SIZE * SIZE * 3) + ((r / 3) * 3 + (c / 3)) * SIZE +
                           (num - 1)};
...
static Column *chooseColumnWithMinSize(DLX *dlx) {
...
static void decodeSolution(int solution[], int k, int board[SIZE][SIZE]) {
...
void solveSudokuDLX(DLX *dlx, int solution[], int k, SudokuSolutions *sols,
  ...
  if (header->next == header) {
    if (sols->count < MAX_SOLUTIONS) {
      ...
      sols->dlx_node_visits[sols->count] = *node_visits;
      sols->count++;
    }
    return;
  }
  ...
  for (Node *r = c->head.down; r != &(c->head); r = r->down) {
    ...
    solveSudokuDLX(dlx, solution, k + 1, sols, node_visits);
